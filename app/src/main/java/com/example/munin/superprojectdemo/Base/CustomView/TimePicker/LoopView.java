...
//有数据进行设置paint,手势
    private void d() {
        ...
        s = (int) ((double) (t * 2) / Math.PI);//半圆，求的高度（直径）
        u = (int) ((double) t / Math.PI);//半径
        v = g + textSize;
        n = (int) (((float) s - l * (float) h) / 2.0F);//奇一半
        o = (int) (((float) s + l * (float) h) / 2.0F);//偶一半
        if (positon == -1) {
            if (isLoop) {
                positon = (arrayList.size() + 1) / 2;//循环的话中间
            } else {
                positon = 0;//不循环的话从头开始
            }
        }
        mCurrentItem = positon;//初始化当前位置
    }
//矩形
    private void e() {
        Rect rect = new Rect();
        for (int i1 = 0; i1 < arrayList.size(); i1++) {
            ...
            paintB.getTextBounds("0000", 0, "0000".length(), rect);//获得字的区域
            int j1 = rect.width();//宽度
